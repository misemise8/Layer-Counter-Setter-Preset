  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    btnCus?.addEventListener('click',()=>{
      ...
      if(toggleWindow?.checked){
        try{ cs.openModelessDialog(url,'PresetConfig','width=600,height=700'); }
        catch(e){ console.warn(e); cs.openURLInDefaultBrowser(configURL); }
      } else {
        iframe.src=url;
    ...
    window.addEventListener('message',e=>{
      if(e.data?.type==='loadPresets'){
        presets=e.data.configs||presets;
        applyPresetUI(presets);
      }
      if(e.data?.type==='savePresets'){
        presets=e.data.configs;
        localStorage.setItem('ffxPresetConfig',JSON.stringify(presets));
        applyPresetUI(presets);
        if(!toggleWindow?.checked){ overlay.style.display='none'; iframe.src=''; }
      }
    });

    document.querySelectorAll('.preset-apply').forEach((b,i)=>{
    ...
    function applyPresetUI(list){
      document.querySelectorAll('.preset-apply').forEach((b,i)=>{
        const c=list[i]||{};
        if(c.path){ b.disabled=false; b.textContent=c.label||c.path.split(/[\\/]/).pop().slice(0,7); }
        else{ b.disabled=true; b.textContent=`Slot ${i+1}`; }
      });
    }
  });
})();